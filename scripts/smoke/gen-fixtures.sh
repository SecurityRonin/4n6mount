...
for f in exfat.img hfsplus.img apfs.img crash.dmp ad1.ad1 aff4-logical.aff4; do
...
# --- VMDK: monolithicSparse (a single self-contained file). The engine hands
# vmdk-core a byte-source, not a path, so a SINGLE-file subformat is required: a
# monolithicFlat descriptor points at a sibling `-flat.vmdk` that the engine's
# byte-source cannot resolve (the fs then reads as absent). monolithicSparse is
# the uncompressed single-file sparse layout vmdk-core parses in one stream. ---
mkfs_with_file "$W/vmdk-inner.raw" "mkfs.ext4 -F -q" "-o loop" "hello from vmdk"
qemu-img convert -O vmdk -o subformat=monolithicSparse "$W/vmdk-inner.raw" "$OUT/test.vmdk"
